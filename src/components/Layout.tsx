...
export default function Layout() {
  ...
  useEffect(() => {
    if (location.pathname === '/maintenance' || location.pathname.startsWith('/admin')) return;
    const check = async () => {
      try {
        const res = await fetch('/.netlify/functions/check-maintenance');
        const { maintenance } = await res.json();
        if (maintenance) navigate('/maintenance', { replace: true });
        return;
      } catch {}
      try {
        const { data } = await supabase.from('site_config').select('maintenance_mode').single();
        if (data?.maintenance_mode) navigate('/maintenance', { replace: true });
      } catch {}
    };
    check();
  }, []);
            ...
            <div className="flex items-center gap-3 lg:ml-4 lg:bg-transparent rounded-lg px-3 py-2 bg-secondary/90 backdrop-blur-sm">
              ...
              {!loading && !user ? (
                ...
                <div className="flex items-center gap-2">
                  <Link to="/admin"
                    className="px-4 py-2 rounded-lg bg-primary text-white font-medium hover:opacity-90 transition-colors text-sm">
                    Admin
                  </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            ...
            <div>
              ...
              <ul className="space-y-3 text-sm">
                ...
                <li>
                  <Link to="/contact" className="flex items-start gap-3 text-white/60 hover:text-white transition-colors">
                    <svg className="w-5 h-5 mt-0.5 text-primary flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
